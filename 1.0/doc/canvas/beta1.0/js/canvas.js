...
/*
	...
	返回数组 index	
*/
canvas.prototype.circle=function(x,y,r,fill,noCacheData){
	this.ctx.beginPath();
...
/*	
	...
	返回数组 index
*/
canvas.prototype.rect=function(x,y,width,height,fill,noCacheData){
	if(!!fill){
...
/*
	...
	返回数组 index 
*/
canvas.prototype.image=function(imageURL,x,y,width,height,noCacheData){
	width=!width?0:width;
...
/*
	...
	返回数组 index
*/
canvas.prototype.line=function(x1,y1,x2,y2,noCacheData){
	this.ctx.beginPath();
...
/*
	...
	返回数组 index
*/
canvas.prototype.polygon=function(xponits,fill,noCacheData){
	if (xponits.length<3) return;
	this.ctx.beginPath();
	for(var i=0;i<xponits.length;i++){
		if(i==0) this.ctx.moveTo(xponits[i].x,xponits[i].y);
		this.ctx.lineTo(xponits[i].x,xponits[i].y);
		if((i+1)==xponits.length) this.ctx.lineTo(xponits[0].x,xponits[0].y);
	}
	this.ctx.stroke();
	this.ctx.closePath();
	if(!!fill) this.ctx.fill();
	if(!!noCacheData) return;
	return this.add({'type':'polygon','data':{'xponits':xponits,'fill':fill,'noCacheData':noCacheData}});
}
/*
	text
	返回数组 index
*/
canvas.prototype.text=function(string,x,y,noCacheData){
	if(this.ctx.font=='10px sans-serif') this.ctx.font='bold 14px sans-serif';
	this.ctx.fillText(string, x, y,false,{'font':this.ctx.font,'color':this.ctx.fillStyle});
	if(!!noCacheData) return;
...
canvas.prototype.display=function(){
	...
	for(var i=0;i<play.length;i++){
		...
		switch(type){
			...
			break;
			case 'polygon' :
				this.polygon(data.xponits,data.fill);
			break;
			default :
