define(function(require, exports, module){
	...
	$.fn.treenav = function(options){                
		...
		//+' <a href="#" type="add" class="btn btn-mini btn-success"><i class="icon-plus-sign" title="添加"></i></a>'
		...
		var findParentItem=function(){ //构造父级列表
			...
			$.each(settings.data,function(i,v){
				...
				if(v[settings.firstview.field]==settings.firstview.value){
					...
					if(!!v.children){
						//ul+='<li><div class="hitarea"><i class="icon-folder-open"></i><a style="font-weight: bold;text-decoration:none;" mid="'+v.id+'" '+field+'>'+v.name+'</a></div><!--{'+v.id+'}--></li>';
						ul+='<div class="line" nid="'+v.id+'"><div class="left"><i class="icon-folder-open"></i>'+v.name+'</div><div class="right">'+right+'</div></div><!--{'+v.id+'}-->';
						var subli=findChildItem(i);
						ul=ul.replace('<!--{'+v.id+'}-->',subli);
					}else{
						ul+='<div class="line" nid="'+v.id+'"><div class="left"><i class="icon-cog"></i>'+v.name+'</div><div class="right">'+right+'</div></div>';
					}
				}
			});
			ul+='</div>';
		...
		var findChildItem=function(id){ //构造子列表
			...
			var ul='<div class="nav_child">';
			$.each(children,function(i,cid){
				var field=findField(device_json[cid]);
				if(!!device_json[cid].children){
					//ul+='<li><div class="hitarea"><i class="icon-folder-open"></i><a mid="'+device_json[cid].id+'" '+field+'>'+device_json[cid].name+'</a></div><!--{'+cid+'}--></li>';
					ul+='<div class="line" nid="'+device_json[cid].id+'"><div class="left"><i class="icon-folder-open"></i>'+device_json[cid].name+'</div><div class="right">'+right+'</div></div><!--{'+cid+'}-->';
					...
					ul+='<div class="line"  nid="'+device_json[cid].id+'"><div class="left"><i class="icon-cog"></i>'+device_json[cid].name+'</div><div class="right">'+right+'</div></div>';
				}
			});
			ul+='</div>';
			return ul;
